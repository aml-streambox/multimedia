 ...
 *
 * Usage:
 *   vfmcap-demo [-d /dev/video_cap] [-n 100] [-c p010|nv12|both] [-H]
 *   vfmcap-demo -p -c nv12 -n 300 -H   # profile NV12 with HDR->SDR conversion
 *
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "d:n:c:o:r:pHh")) != -1) {
        switch (opt) {
        case 'd':
            device = optarg;
            break;
        ...
        default:
            fprintf(stderr,
                "Usage: %s [-d device] [-n frames] [-c p010|nv12|both] [-o outfile] [-r rawfile] [-p] [-H]\n"
                ...
                "  -H  Enable HDR BT.2020 PQ -> SDR BT.709 conversion (NV12 only)\n",
                argv[0]);
            return (opt == 'h') ? 0 : 1;
        }
    }
    ...
    if (do_hdr) fprintf(stderr, "HDR: BT.2020 PQ -> SDR BT.709 (NV12 only)\n");
    ...
    /* Enable HDR->SDR conversion if requested */
    if (do_hdr) {
